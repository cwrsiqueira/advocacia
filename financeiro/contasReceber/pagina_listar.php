...
<table class="table table-responsive-sm table-sm mt-3" tabelas>
	...
	$ent_ants = $pdo->query("SELECT SUM(valor) as entradas FROM lancamentos WHERE tipo = 'receber' AND data_vencimento < '$dataInicial'");

	$venc_futs = $pdo->query("SELECT SUM(valor) as venc_fut FROM lancamentos WHERE tipo = 'receber' AND data_vencimento > '$dataFinal'");
	
	$ent_totaiss = $pdo->query("SELECT SUM(valor) as entradas_totais FROM lancamentos WHERE tipo = 'receber' AND data_vencimento <= '$dataFinal'");
	...
	$res = $pdo->query("SELECT * from lancamentos where movimento LIKE '%$txtbuscar%' AND (data_vencimento >= '$dataInicial' and data_vencimento <= '$dataFinal') AND tipo = 'receber' order by data_vencimento, tipo DESC, id");
		...
		<tr class="linha'.$id.'">
			...
			<td class="nao-imprimir">
				<div class="d-flex justify-content-between">
					...
					<a title="Liquidar Lançamento" href="" id="liquidar_lancamento" data-id="'.$id.'" data-tipo="receber">
